...
static XkbStateRec kbd_global_state;
...
buttonpress(XEvent *e)
{
	...
	if (ev->window == selmon->barwin) {
		...
		if (i < LENGTH(tags)) {
			...
			click = ClkStatusText;
		else if (ev->x > x + TEXTW(selmon->ltsymbol) && ev->x < x + TEXTW(selmon->ltsymbol) + sbw) {
			unsigned short i1 = 0;
			unsigned short x1 = x + TEXTW(selmon->ltsymbol);
			do
...
focus(Client *c)
{
	...
	if (c) {
		...
		if (keyboardlayout)
			XkbLockGroup(dpy, XkbUseCoreKbd, kbd_global_state.group);
...
setup(void)
{
	...
	if (keyboardlayout)
		XkbGetState(dpy, XkbUseCoreKbd, &kbd_global_state);
	/* init atoms */
